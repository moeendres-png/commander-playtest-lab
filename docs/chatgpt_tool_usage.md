...
The only authorized OpenCode model is `opencode-go/muse-spark-1.3-contributor`. Allowed effort is `high` or `xhigh` only. `high` is the minimum; `xhigh` is the default and preferred project effort. `medium` and all lower effort variants are not authorized for Commander Simulator Next OpenCode execution. Use XHigh by default for substantial implementation, debugging, audits, qualification, semantic integration, multi-file remediation and any task where additional reasoning could materially improve correctness. When uncertain between High and XHigh, choose XHigh. Use High only for clearly bounded, local, mechanical or low-ambiguity work where additional reasoning is not expected to materially improve the result. The repository root `opencode.json` enforces the current OpenCode provider/model/effort policy.
