...
TOOL_DEFINITIONS: tuple[ToolDefinition, ...] = (
    ...
    ToolDefinition("build_optimization_context", "Load the current read-only deck, inventory, allocation, opponent, pilot, politics, coverage and engine context.", BuildOptimizationContextInput, "build_optimization_context"),
    ToolDefinition("generate_candidate_swaps", "Generate whole-deck candidate swaps without applying changes.", GenerateCandidateSwapsInput, "generate_candidate_swaps"),
    ToolDefinition("generate_candidate_packages", "Generate and diagnose candidate package changes without applying them.", GenerateCandidatePackagesInput, "generate_candidate_packages"),
    ToolDefinition("optimize_deck_against_meta", "Screen and structurally validate deck candidates against the configured opponent meta.", OptimizeDeckAgainstMetaInput, "optimize_deck_against_meta"),
    ToolDefinition("optimize_multiple_decks_with_allocation", "Optimize multiple decks while resolving singleton allocation conflicts by user priority.", OptimizeMultipleDecksWithAllocationInput, "optimize_multiple_decks_with_allocation"),
    ToolDefinition("validate_swap", "Validate one or more swaps through formal, paired structural, holdout, sensitivity and red-team gates.", ValidateSwapInput, "validate_swap"),
    ToolDefinition("validate_package_change", "Validate a package change and its density, failure modes and structural swap evidence.", ValidatePackageChangeInput, "validate_package_change"),
    ToolDefinition("validate_land_change", "Validate a land change with land-specific and structural gates.", ValidateLandChangeInput, "validate_land_change"),
    ToolDefinition("validate_mulligan_policy", "Compare two mulligan policies with common random numbers and model labels.", ValidateMulliganPolicyInput, "validate_mulligan_policy"),
    ToolDefinition("run_multifidelity_comparison", "Run the available formal, structural, Tactical Oracle coverage and external-engine gates without overstating evidence.", RunMultifidelityComparisonInput, "run_multifidelity_comparison"),
    ToolDefinition("run_engine_backed_matchup", "Run a real XMage or Forge matchup only when a provider process is available; otherwise return blocked evidence.", RunEngineBackedMatchupInput, "run_engine_backed_matchup"),
    ToolDefinition("run_robustness_suite", "Run structural holdout, pilot, politics and pod-size robustness diagnostics.", RunRobustnessSuiteInput, "run_robustness_suite"),
    ToolDefinition("run_rules_coverage_gate", "Check card and interaction coverage and the highest actually executed validation level.", RunRulesCoverageGateInput, "run_rules_coverage_gate"),
    ToolDefinition("rank_variants", "Rank variants by worst-case, paired effect and executed rules coverage.", RankVariantsInput, "rank_variants"),
    ToolDefinition("explain_recommendation", "Explain a recommendation, tradeoffs, truth boundary and remaining uncertainty.", ExplainRecommendationInput, "explain_recommendation"),
    ToolDefinition("export_recommendation_evidence", "Export recommendation evidence without modifying canonical data.", ExportRecommendationEvidenceInput, "export_recommendation_evidence"),
    ToolDefinition("create_deck_improvement_report", "Create a complete model-labeled deck-improvement report without applying changes.", CreateDeckImprovementReportInput, "create_deck_improvement_report"),
