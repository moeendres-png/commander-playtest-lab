...
from commander_lab.models.whole_deck_tooling import WholeDeckDecisionPrepareInput, WholeDeckDecisionRunInput
...
def install_whole_deck_public_integration() -> None:
    ...
    def prepare(self: Any, request: Any):
        ...
        return self._invoke("deck_decision_prepare", request, lambda: prepare_whole_deck(self, request), deck_ids=(request.deck_id,))

    def run(self: Any, request: Any):
        if getattr(request, "comparison_mode", "swap") != "whole_deck":
            return original_run(self, request)
        return self._invoke("deck_decision_run", request, lambda: run_whole_deck(self, request), deck_ids=(request.deck_id,), seed=request.seed, iterations=request.iterations)
    ...
    for definition in registry_module.PUBLIC_TOOL_DEFINITIONS:
        if definition.name == "deck_decision_prepare":
            definitions.append(ToolDefinition("deck_decision_prepare", "Validate current truth and prepare a legacy swap decision or bounded Whole-Deck designs without applying them.", WholeDeckDecisionPrepareInput, "deck_decision_prepare"))
        elif definition.name == "deck_decision_run":
            definitions.append(ToolDefinition("deck_decision_run", "Run one paired structural comparison for a legacy swap or prepared Whole-Deck finalist without applying it.", WholeDeckDecisionRunInput, "deck_decision_run"))
        else:
