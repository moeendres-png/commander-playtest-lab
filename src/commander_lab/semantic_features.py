...
def self_token_creation(oracle_text: str | None) -> bool:
    ...
    return bool(
        "you create" in text
        or re.search(r"\bcreate (?:a|an|one|two|three|four|x|that many|those) [^.]* token", text)
    )
...
def self_mana_semantics(oracle_text: str | None, type_line: str | None) -> tuple[bool, bool]:
    ...
    triggered_mana = bool(
        is_permanent
        and re.search(r"\b(?:whenever|at the beginning|when)\b[^.]{0,160}\badd\b", text)
    )
    repeatable = is_land or activated_mana or triggered_mana
    land_to_battlefield = bool(
        "search your library" in text and "land" in text and "battlefield" in text
    ) or bool("put a land card" in text and "onto the battlefield" in text)
