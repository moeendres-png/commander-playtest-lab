...
from .models import DeckDesignPolicy, ManaBasePolicy, PolicyId
...
def _land_bounds(policy: DeckDesignPolicy) -> tuple[int, int, int, int]:
    ...
    hard_low = (
        0 if corridor.hard_minimum is None else max(0, int(round(corridor.hard_minimum)))
    )
    hard_high = (
        98 if corridor.hard_maximum is None else min(98, int(round(corridor.hard_maximum)))
    )
...
def whole_deck_mana_summary(
    ...
    t1 = {
        key: int(value) for key, value in sorted(analysis.t1_untapped_land_sources.items())
    }
    ...
    return {
        ...
        "average_nonland_mv": (
            fmean(card.mana_value for card in nonlands) if nonlands else 0.0
        ),
...
def mana_soft_score(summary: dict[str, object], policy: ManaBasePolicy) -> float:
    colored = Counter(
        {str(k): int(v) for k, v in dict(summary.get("colored_sources", {})).items()}
    )
    ...
    t1 = Counter(
        {
            str(k): int(v)
            for k, v in dict(summary.get("t1_untapped_land_sources", {})).items()
        }
    )
    ...
    tapped_penalty = max(0, tapped - policy.preferred_maximum_tapped_lands) / max(
        1, land_count
    )
    basic_penalty = 0.0
