...
class _ManaNeighborhoodMixin(SearchEngineBase):
    def _land_balance_proposal(
        ...
        if decrease and lands:
            ...
            additions = self._candidate_additions(
                current, lambda card: card.profile.is_land and not card.is_basic
            )
        additions.sort(
            key=lambda card: (
                (
                    self._land_quality(card)
                    if card.profile.is_land
                    else self._utility[card.oracle_name]
                )
                + rng.random() * 0.2,
                card.oracle_name,
            ),
    ...
    def _basic_mix_proposal(
        ...
        nonbasic_lands = [
            ...
            if self.context.cards[name].profile.is_land and name not in SEARCH_BASIC_LANDS
        ]
        move_to_nonbasic = len(basic_positions) >= self.config.minimum_neighborhood_changes and (
            not nonbasic_lands or rng.random() < 0.6
        )
        k = min(
