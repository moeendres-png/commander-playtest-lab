...
data_app = typer.Typer(
    no_args_is_help=True, help="Canonical Drive-to-program data synchronization audit"
)
...
def data_sync(
    dry_run: bool = typer.Option(
        False, "--dry-run", help="Report changes without mutating prepared imports."
    ),
    root: Path = typer.Option(Path.cwd(), exists=True, file_okay=False, dir_okay=True),
) -> None:
...
def demo_phase5(
    ...
    typer.echo(
        json.dumps(
            run_phase5_demo(root, iterations=iterations, seed=seed), indent=2, ensure_ascii=False
        )
    )
