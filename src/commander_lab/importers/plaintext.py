        ...
        *,
        ...
        for row_number, raw_line in enumerate(text.splitlines(), start=1):
            ...
            try:
                ...
                raise ImportErrorWithContext(str(exc), source=source_path, row=row_number) from exc
            entries.append(
                DeckEntry(
        ...
        uses_partner = (
            options.uses_partner if options.uses_partner is not None else len(commander_names) == 2
        )
        ...
        return Deck(
            ...
            source=self.source_ref(
                source_path or options.source_name or "inline.txt", source_type="plaintext"
            ),
        )
