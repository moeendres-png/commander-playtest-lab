...
class CounterfactualReplayLab:
    ...
    def run(
        ...
        *,
        ...
        if engine_mode == CounterfactualEngineMode.TACTICAL_ORACLE:
            ...
            for label, action in (
                ...
                if not action.tactical_rule:
                    raise CounterfactualError(
                        "tactical_oracle mode requires recorded tactical_rule metadata "
                        "for both actions"
                    )
                try:
        ...
        warnings = [
            "Counterfactuals are model alternatives, not facts about what historically "
            "would have happened.",
    ...
    def report(result: CounterfactualResult, target: Path) -> None:
        lines = [
            ...
            "## Boundary",
            "",
            "This is a counterfactual model alternative. It is not evidence that history "
            "would certainly have unfolded this way.",
        ]
