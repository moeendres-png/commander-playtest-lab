...
class WholeDeckDecisionPrepareInput(FrozenModel):
    ...
    whole_deck_policies: tuple[str, ...] = ("OWNED_POOL_NEUTRAL", "META_LIGHT", "LOW_LAND_HIGH_VELOCITY", "RESILIENT_COMMANDER_INDEPENDENT", "INTERACTION_HEAVY_LOCAL_META")
...
class WholeDeckDecisionRunInput(FrozenModel):
    ...
    def validate_comparison_mode(self) -> "WholeDeckDecisionRunInput":
        ...
        elif not self.prepared_design_path or not self.whole_deck_variant_id:
            raise ValueError("whole_deck comparison requires prepared_design_path and whole_deck_variant_id")
        return self
