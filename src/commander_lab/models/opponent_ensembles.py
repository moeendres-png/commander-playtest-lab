...
class UncertaintyDimension(FrozenModel):
    ...
    source_ids: tuple[str,...]=()
    confidence: float=Field(default=.25,ge=0,le=1)
...
class OpponentVariant(FrozenModel):
    ...
    source_ids: tuple[str,...]=()
    confidence: float=Field(default=.25,ge=0,le=1)
    weight: VariantWeight=Field(default_factory=lambda:VariantWeight(mode=EnsembleWeightMode.UNWEIGHTED))
    synthetic: bool=True
    ...
    def colors_and_statuses(self)->"OpponentVariant":
        ...
        overlap={c.card_name for c in self.known_cards}&{c.card_name for c in self.assumed_cards}
        if overlap: raise ValueError(f"cards cannot be both known and assumed: {sorted(overlap)}")
        return self
...
class OpponentEnsemble(MutableModel):
    ...
    def validate_ensemble(self)->"OpponentEnsemble":
        if not self.variants: raise ValueError("ensemble requires at least one variant")
        ids=[v.variant_id for v in self.variants]
        if len(ids)!=len(set(ids)): raise ValueError("variant IDs must be unique")
        if any(v.commander!=self.commander for v in self.variants): raise ValueError("all variants require the ensemble commander")
        ...
        required_cards={c.value for c in self.observed_constraints if c.kind=='card_present' and c.required}
        prohibited={c.value for c in self.observed_constraints if c.kind=='card_absent' and c.required}
        for v in self.variants:
            known={c.card_name for c in v.known_cards}; assumed={c.card_name for c in v.assumed_cards}
            if not required_cards.issubset(known): raise ValueError(f"variant {v.variant_id} misses observed known cards")
            if prohibited & (known|assumed): raise ValueError(f"variant {v.variant_id} contradicts observed absence")
        values=[v.weight.value for v in self.variants]
        if self.weight_mode in {EnsembleWeightMode.UNWEIGHTED, EnsembleWeightMode.EQUAL, EnsembleWeightMode.WORST_CASE}:
            if any(x is not None for x in values):
                raise ValueError("unweighted, equal, and worst-case ensembles cannot contain numeric weights")
        else:
            if any(x is None for x in values): raise ValueError("weighted ensemble requires every weight")
            if abs(sum(x or 0 for x in values)-1.0)>1e-6: raise ValueError("variant weights must sum to one")
        return self
