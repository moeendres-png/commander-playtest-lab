...
class ProjectContextSnapshot:
    ...
    engine_version: str
    global_active_own_deck_ids: tuple[str, ...]
    runtime_loaded_deck_ids: tuple[str, ...]
    optimization_target_ids: tuple[str, ...]
    unresolved_operational_baseline_ids: tuple[str, ...]
    # Backward-compatible runtime aliases. They intentionally do NOT mean global ownership.
...
def _string_tuple(payload: dict[str, Any], key: str, *, required: bool = True) -> tuple[str, ...]:
    raw = payload.get(key)
    if raw is None and not required:
        return ()
    if not isinstance(raw, list):
        raise ProjectContextError(f"project scope field {key} must be a list")
    values = tuple(str(value) for value in raw)
    if len(values) != len(set(values)):
        raise ProjectContextError(f"project scope field {key} contains duplicate ids")
    return values
...
def _load_scope(
    ...
    global_active = _string_tuple(scope, "global_active_own_decks")
    runtime = _string_tuple(scope, "runtime_loaded_decks")
    optimization = _string_tuple(scope, "optimization_targets")
    unresolved = _string_tuple(scope, "unresolved_operational_baselines", required=False)
    historical = _string_tuple(scope, "historical_own_decks", required=False)
    focus = scope.get("primary_deckbuilding_focus")

    if not global_active:
        raise ProjectContextError("global active own-deck scope is missing or empty")
    if not runtime:
        raise ProjectContextError("runtime-loaded deck scope is missing or empty")
    if not optimization:
        raise ProjectContextError("optimization target scope is missing or empty")
    if not isinstance(focus, str) or not focus:
        raise ProjectContextError("primary deckbuilding focus is missing")

    global_set = set(global_active)
    runtime_set = set(runtime)
    optimization_set = set(optimization)
    unresolved_set = set(unresolved)
    historical_set = set(historical)
    frozen = set(_string_tuple(scope, "frozen_opponent_only_decks"))

    if not runtime_set.issubset(global_set):
        raise ProjectContextError("runtime-loaded decks must be globally active own decks")
    if not optimization_set.issubset(global_set):
        raise ProjectContextError("optimization targets must be globally active own decks")
    ...
    if global_set & historical_set:
        raise ProjectContextError("a deck cannot be both globally active and historical")
    if frozen & global_set:
        raise ProjectContextError("a frozen opponent-only deck cannot be an active own deck")
    ...
    return global_active, runtime, optimization, unresolved, historical, focus, scope
...
def _validate_release_projection(
    ...
    *,
    ...
    if not isinstance(released, dict):
        raise ProjectContextError("released_allocations must be a mapping")
    if unresolved and released:
        raise ProjectContextError(
            "active unresolved deck allocations must not be treated as released availability"
        )
...
def _validate_playstyle(payload: dict[str, Any]) -> str:
    ...
    return str(preference_type)


def _runtime_deck_hashes(
    root: Path,
    ...
    *,
    ...
    manifest_global = tuple(str(value) for value in manifest.get("global_active_own_decks", []))
    manifest_runtime = tuple(str(value) for value in manifest.get("runtime_loaded_decks", []))
    legacy_active = tuple(str(value) for value in manifest.get("active_own_decks", []))
    if manifest_global != global_active:
    ...
    for deck_id in unresolved:
        if deck_id in decks:
            raise ProjectContextError(
                f"unresolved current operational baseline unexpectedly appears in runtime manifest: {deck_id}"
            )
    ...
    for deck_id in runtime:
        ...
        if (
            ...
            raise ProjectContextError(f"runtime-loaded deck has unsafe normalized file: {deck_id}")
        deck_path = root / "data/decks" / normalized_file
        try:
            deck = Deck.model_validate(_load_json(deck_path))
        except ValueError as exc:
            raise ProjectContextError(f"runtime-loaded deck file is invalid: {deck_id}") from exc
        if deck.deck_id != deck_id:
            raise ProjectContextError(f"runtime-loaded deck file identity mismatch: {deck_id}")
        if deck.deck_hash != digest:
            raise ProjectContextError(f"runtime-loaded deck embedded hash mismatch: {deck_id}")
        if compute_deck_hash(deck) != digest:
            raise ProjectContextError(f"runtime-loaded deck content hash mismatch: {deck_id}")
        if raw.get("total_cards") != deck.total_cards:
            raise ProjectContextError(f"runtime-loaded deck total-card count mismatch: {deck_id}")
        if raw.get("library_cards") != deck.library_cards:
            raise ProjectContextError(f"runtime-loaded deck library-card count mismatch: {deck_id}")
        if tuple(raw.get("commanders", ())) != tuple(deck.commander.commanders):
            raise ProjectContextError(f"runtime-loaded deck commander identity mismatch: {deck_id}")
...
def load_project_context(root: str | Path) -> ProjectContextSnapshot:
    ...
    global_active, runtime, optimization, unresolved, historical, focus, scope = _load_scope(
        paths["active_scope"]
    )
    _validate_live_scope_projection(scope, runtime=runtime, historical=historical)
    release_projection = _load_json(paths["inactive_deck_releases"])
    ...
    if unexpected_primary:
        raise ProjectContextError(
            f"current primary scenarios reference non-runtime own decks: {unexpected_primary}"
        )
    allowed_historical_references = set(historical) | set(global_active)
    ...
    runtime_deck_hashes, runtime_deck_paths = _runtime_deck_hashes(
        root_path,
        deck_manifest,
        global_active=global_active,
        runtime=runtime,
    ...
    for deck_id, deck_path in runtime_deck_paths.items():
    ...
    identity_payload = {
        ...
        "global_active_own_deck_ids": list(global_active),
        "runtime_loaded_deck_ids": list(runtime),
        "optimization_target_ids": list(optimization),
        "unresolved_operational_baseline_ids": list(unresolved),
        "historical_own_deck_ids": list(historical),
        "primary_deckbuilding_focus": focus,
        "runtime_deck_hashes": dict(runtime_deck_hashes),
    ...
    return ProjectContextSnapshot(
        ...
        software_version=__version__,
        engine_version=ENGINE_VERSION,
        global_active_own_deck_ids=global_active,
        runtime_loaded_deck_ids=runtime,
        optimization_target_ids=optimization,
        unresolved_operational_baseline_ids=unresolved,
        active_own_deck_ids=runtime,
        historical_own_deck_ids=historical,
        primary_deckbuilding_focus=focus,
        active_deck_hashes=runtime_deck_hashes,
