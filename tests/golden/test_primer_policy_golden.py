...
def test_curated_policy_golden_scenarios_do_not_regress() -> None:
    ...
    cases = (("rogshai/current","rogshai","data/primer_rules/policies/rogshai_current_policy-1.0.1.json"),)
    improved = 0
    for deck_id, strategy, policy_path in cases:
        policy = CompiledPilotPolicy.model_validate_json((ROOT / policy_path).read_text(encoding="utf-8"))
        ...
        results = compiler.evaluate_policy(base_pilot=pilot,policy=policy,scenarios=scenarios,deck_cards=tuple(card.oracle_name for card in decks[deck_id].cards))
