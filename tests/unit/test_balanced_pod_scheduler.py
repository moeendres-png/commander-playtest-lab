...
def test_full_cycle_covers_all_triplets_and_balances_exposure_and_own_seats(repo_root) -> None:
    ...
    assert scheduler.combinations_per_cycle == 455
    assert len(scenarios) == 455
    assert len({tuple(sorted(row.opponent_deck_ids)) for row in scenarios}) == 455
    ...
    assert set(report["games_per_opponent"].values()) == {91}
    assert report["rogshai_seat_counts"] == {"1": 114, "2": 114, "3": 114, "4": 113}
...
def test_second_cycle_preserves_full_coverage_with_new_scenarios(repo_root) -> None:
    ...
    assert report["incomplete_remainder_games"] == 0
    assert set(report["games_per_opponent"].values()) == {182}
