...
def test_g_owned_pool_neutral_is_control_blind_until_finalist_freeze():
    ...
    config = neutral_config(77)
    with_control = WholeDeckSearchEngine(context, policy, config=config).run(
        **{"current_" + "control": baseline}
    )
    ...
    assert (
        with_control.control_used_as_search_prior is False
        and with_control.control_variant_id is not None
    )
    ...
    assert all(
        v.provenance["current_deck_membership_prior_used"] is False for v in with_control.variants
    )
