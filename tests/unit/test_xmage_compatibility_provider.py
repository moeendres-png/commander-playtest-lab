...
def test_b1_bridge_is_present_but_remains_fail_closed(
    ...
    assert (
        bridge_root
        / "src/main/java/org/commanderlab/xmage/Main.java"
    ).is_file()
    assert (
        bridge_root
        / "src/main/java/org/commanderlab/xmage/JsonlBridge.java"
    ).is_file()
    assert (
        bridge_root
        / "src/main/java/org/commanderlab/xmage/XmageProvider.java"
    ).is_file()
    ...
    windows = (
        repo_root / "scripts/bootstrap_engine_windows.ps1"
    ).read_text(encoding="utf-8")
    linux = (
        repo_root / "scripts/bootstrap_engine_linux.sh"
    ).read_text(encoding="utf-8")
    ...
    config = json.loads(
        (repo_root / "config/rules_engines.json").read_text(
            encoding="utf-8"
        )
    )
