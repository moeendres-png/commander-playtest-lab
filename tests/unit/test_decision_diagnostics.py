...
def _load(name):
    return DecisionDiagnosticEngine.load(ROOT / f"data/diagnostics/synthetic_cases/{name}.json")
...
def test_known_cause_incomplete_package():
    assert (
        DecisionDiagnosticEngine().classify(_load("incomplete_package"), "Support Card").hypothesis
        == FailureCause.PACKAGE_IS_INCOMPLETE
    )


def test_known_cause_wrong_opponent():
    assert (
        DecisionDiagnosticEngine().classify(_load("wrong_opponent"), "Test Card").hypothesis
        == FailureCause.OPPONENT_MODEL_IS_WRONG
    )


def test_known_cause_random_variance():
    assert (
        DecisionDiagnosticEngine().classify(_load("high_variance"), "Test Card").hypothesis
        == FailureCause.RANDOM_VARIANCE
    )


def test_known_cause_wrong_structural_abstraction():
    assert (
        DecisionDiagnosticEngine().classify(_load("wrong_structural"), "Test Card").hypothesis
        == FailureCause.SIMULATION_ABSTRACTION_IS_WRONG
    )


def test_factor_comparison_and_next_experiment():
    engine = DecisionDiagnosticEngine()
    dataset = _load("poor_pilot")
...
def test_integrated_ten_step_smoke(tmp_path):
    ...
    assert all(
        row.source_paths and row.source_hashes and row.validation_level for row in report.steps
    )
    ...
    try:
        StructuralSimulator(decks).simulate(
            StructuralMatchConfig(
                ...
                limits=StructuralAbortLimits(max_turns=5),
            ),
            event_log_path=log,
            capture_events=True,
        )
