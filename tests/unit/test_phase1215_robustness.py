...
def test_structural_policy_mapping_uses_real_pilot_weights_and_truth_boundary(repo_root: Path) -> None:
...
def test_current_uncertainty_registry_preserves_known_unknown_counts(repo_root: Path) -> None:
    ...
    assert len([r for r in registry['opponent_variants'] if r['deck_id'] == 'opponent/doom-prevails-precon']) == 3
...
def test_structural_policy_tournament_executes_real_simulator(repo_root: Path) -> None:
    ...
    result = run_structural_policy_tournament(
        repo_root,
        config=PolicyTournamentConfig(pod_sizes=(3,), iterations_per_scenario=1, max_turns=10, rounds=2),
...
def test_structural_policy_tournament_is_worker_independent(repo_root: Path) -> None:
    from commander_lab.robustness import run_structural_policy_tournament, PolicyTournamentConfig
    kwargs = {
        "root": repo_root,
        "deck_ids": ("korvold/current",),
        "pilot_profiles": ("weak", "strong"),
    ...
    one = run_structural_policy_tournament(
        config=PolicyTournamentConfig(seed=99, rounds=2, pod_sizes=(3,), iterations_per_scenario=1, max_turns=8, workers=1),
        **kwargs,
    )
    two = run_structural_policy_tournament(
        config=PolicyTournamentConfig(seed=99, rounds=2, pod_sizes=(3,), iterations_per_scenario=1, max_turns=8, workers=2),
        **kwargs,
    )
