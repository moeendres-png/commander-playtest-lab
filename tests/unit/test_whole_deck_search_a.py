...
def test_a_six_plus_coordinated_changes_escape_single_swap_valley():
    ...
    engine = WholeDeckSearchEngine(
        context, get_policy(PolicyId.OWNED_POOL_NEUTRAL), config=neutral_config(7)
    )
    before = engine.evaluate_mainboard(baseline)
    proposal, cuts, inclusions = engine.propose(
        baseline, WholeDeckNeighborhood.ENGINE_PACKAGE, random.Random(11)
    )
    after = engine.evaluate_mainboard(proposal)
    assert len(cuts) >= 6
    assert len(inclusions) >= 6
    assert set(inclusions) == {f"Engine Piece {index}" for index in range(1, 7)}
    assert after.objective_prior > before.objective_prior
    one_swap = tuple("Engine Piece 1" if card == "Filler 1" else card for card in baseline)
    assert engine.evaluate_mainboard(one_swap).objective_prior < before.objective_prior
