...
def test_process_manager_honors_log_rotation_environment(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    monkeypatch.setenv("ENGINE_LOG_MAX_BYTES", "64")
    monkeypatch.setenv("ENGINE_LOG_BACKUP_COUNT", "2")
    manager = EngineProcessManager(EngineRuntimeConfig(), root=tmp_path)

    assert manager.log_max_bytes == 64
    assert manager.log_backup_count == 2
