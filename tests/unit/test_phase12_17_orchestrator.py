...
def test_openai_stdio_config_is_read_only_and_truthful() -> None:
    ...
    config = json.loads((Path(__file__).resolve().parents[2] / "config/openai_mcp_stdio.json").read_text())
    ...
    assert config["openai_agents_sdk"]["live_sdk_test_status"] == "blocked_not_installed_in_current_runtime"
