...
def names(deck, wanted):
    ...
    for name in wanted:
        i = used.get(name, 0)
        result.append(pool[name][i])
        used[name] = i + 1
    return tuple(result)


def test_london_mulligan_uses_free_first_multiplayer_and_commanders_stay_out(
    repo_root: Path,
) -> None:
...
def test_tool_surface_runs_and_generates_non_absolute_rule(repo_root: Path, tmp_path: Path) -> None:
    ...
    eval_response = service.evaluate_opening_hand(
        EvaluateOpeningHandInput(
...
def test_context_dimensions_change_model_score(repo_root: Path) -> None:
    ...
    control = base.model_copy(
        update={
            "seat_position": 4,
            "starting_player": False,
            "pilot_profile_id": "RogShaiControlPilot",
            "game_plan": MulliganGamePlan.CONTROL,
        }
    )
    tempo = base.model_copy(
        update={
            "seat_position": 1,
            "starting_player": True,
            "pilot_profile_id": "RogShaiTempoPilot",
            "game_plan": MulliganGamePlan.FAST_PRESSURE,
        }
    )
    assert (
        lab.evaluate(deck, hand, MulliganPolicyName.MATCHUP_ORIENTED, control).score
        != lab.evaluate(deck, hand, MulliganPolicyName.MATCHUP_ORIENTED, tempo).score
    )
...
def test_full_followup_and_overfitting_contexts_are_executed(repo_root: Path) -> None:
    ...
    assert {row.context_kind for row in result.overfitting_validation} == {
        "primary_pod",
        "holdout_pod",
        "opponent_ensemble",
        "pilot_profile",
