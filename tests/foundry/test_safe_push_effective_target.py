...
Fix-after (this file): exact owner/repo identity on BOTH the configured
fetch URL and the Git-expanded effective push URL
(``git remote get-url --push --all``); exactly-one-record rules;
insteadOf/pushInsteadOf presence guards in every scope; pre-write re-check;
no URL/credential/command-output echoes.
...
def test_explicit_canonical_pushurl_retained(rig: dict) -> None:
...
def test_canonical_pushurl_file_scheme_variant(rig: dict) -> None:
    _git(
...
def test_divergent_pushurl_rejected_dry_and_actual(rig: dict) -> None:
    ...
    assert "WRONG_REMOTE" in dry.stderr
    assert "effective push target" in dry.stderr
    push = _run_push(rig)
    assert push.returncode == 2
    assert "WRONG_REMOTE" in push.stderr
    assert not _remote_has(rig["evil_b"], rig["wt"], "refs/heads/test/ws")
    assert not _remote_has(rig["remote_a"], rig["wt"], "refs/heads/test/ws")
