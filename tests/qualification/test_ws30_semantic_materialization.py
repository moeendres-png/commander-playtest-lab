...
def load(name): return json.loads((MAT/name).read_text(encoding='utf-8'))
...
def test_materialization_file_checksum_and_subset_counts():
    ...
    actual=hashlib.sha256((MAT/'SEMANTIC_FIXTURE_MATERIALIZATION_v1.json').read_bytes()).hexdigest()
...
def test_validator_strict_against_repository_sources():
    subprocess.run([sys.executable,str(ROOT/'scripts/validate_ws30_materialization.py'),'--repo-root',str(ROOT)],check=True)
