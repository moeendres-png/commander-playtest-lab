#!/usr/bin/env python3
"""WS53 converged decision-sequence provider overlay v1 (WS53-owned, qualification-only).

Chained patch applied AFTER candidate-qualification/ws48-forge-v1.0.5/
ws48_behavior_provider_overlay.py (post-Repair-01) onto the ephemeral generated
GPL-side provider (Ws23ForgeVerticalProvider.java). Never touches pinned Forge
source, shared scripts, WS48-owned files, WS50-owned files, or another
workstream's files.

Converged from WS50 donor ws50_provider_overlay.py (e636e705) per adjudicated
matrix WS53_CONVERGENCE_MATRIX.json + WS53_CONVERGENCE_ADJUDICATION.md (XHIGH PASS
with binding corrections):

PORTED (SAFE_ORTHOGONAL_PORT):
1. discard transport upgrade (WS50-D03): chooseCardsToDiscardToMaximumHandSize
   offers per-card WS48:OPT labels projected from the engine's own hand (same
   pattern as the WS48 chooseCardsForEffect transport). Zero/ambiguous cases
   still fail closed harness-side.
2. per-frame decision metadata (WS50-D04): additive DECISION_FRAME payload fields
   only (old drivers ignore unknown fields):
   ...
   - observations: per-principal redacted views plus sha256 fingerprints.
   Java helper/logic bodies are byte-identical to the WS50 PASS-proven artifact
   (fidelity wins over renaming; ownership is this file + its messages).

OMITTED (SUPERSEDED_BY_WS48_REPAIR01, WS50-D02):
- WS50-ADAPTER-REPAIR-01 single-add hunk. The current-tree WS48 overlay already
  carries the strictly stronger Repair-01 (zero-add projection + cardinality
  guard + index range check + priority_binding record). The doubled-add anchor
  does not exist in the converged overlay output; attempting the hunk would
  fail loud. This overlay instead ASSERTS the doubled-add text absent
  (WS53_OVERLAY_REPAIR01_RESURRECTED) as defense-in-depth: porting it would
  resurrect the forbidden double-population failure.
...
def once(text: str, old: str, new: str, label: str) -> str:
    ...
    n = text.count(old)
    if n != 1:
        raise SystemExit(f"WS53_OVERLAY_ANCHOR:{label}:expected=1:found={n}")
    return text.replace(old, new, 1)


# WS53 Repair-01 absence assertion (defense-in-depth, NOT a repair).
# The converged input (repaired WS48 overlay output) must contain exactly one
# nativeOptions.add(sa) per seen SpellAbility (base generator) and zero from
# the overlay projection. If the doubled-add text ever matches, the lineage
# has regressed and the build must stop here instead of misbinding selections.
DOUBLED_ADD_PROBE = """                            nativeOptions.add(sa);
...
def main() -> int:
    ...
    # Require the repaired WS48 overlay first (chained patch, WS48 files untouched).
    for marker in ("WS48:ATTACK:attacker=", "ws48FinalizeDeclaredBands",
        ...
        if marker not in p:
            raise SystemExit(f"WS53_OVERLAY_PREREQ_MISSING:{marker}")
    # Repair-01 supersession: the doubled-add text must be ABSENT. If present,
    # the lineage regressed; stop here rather than misbind or silently repair.
    if DOUBLED_ADD_PROBE in p:
        raise SystemExit("WS53_OVERLAY_REPAIR01_RESURRECTED:doubled nativeOptions.add(sa) present")
    # Repair-01 guards from the WS48 overlay must be present (converged input).
    for marker in ("WS48_SELECTION_EXECUTION_CARDINALITY_MISMATCH",
                   "WS48_SELECTION_EXECUTION_INDEX_OUT_OF_RANGE",
                   "priority_binding"):
        if marker not in p:
            raise SystemExit(f"WS53_OVERLAY_REPAIR01_GUARD_MISSING:{marker}")
    p = once(p, DISCARD_OLD, DISCARD_NEW, "discard labeled transport")
    p = once(p, STATIC_HELPERS_ANCHOR, STATIC_HELPERS_ADD, "ws53 frame helpers")
    p = once(p, CHOOSE_OLD, CHOOSE_NEW, "ws53 frame capture")
    p = once(p, OPTS_OLD, OPTS_NEW, "ws53 frame payload")
    ...
    if missing:
        raise SystemExit(f"WS53_OVERLAY_INCOMPLETE:{missing}")
    if DOUBLED_ADD_PROBE in p:
        raise SystemExit("WS53_OVERLAY_REPAIR01_NOT_PRESERVED")
    print("WS53_CONVERGED_DECISION_SEQUENCE_PROVIDER_OVERLAY_V1=PASS")
