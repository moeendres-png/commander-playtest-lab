#!/bin/bash
# WS53 converged sequence provider build (authored; WS50 build scripts are
# mechanism donors only, never copied verbatim: absolute paths differ).
# Order (binding, XHIGH Q6.6): generate -> ws25 -> WS48 overlay (repaired) ->
# WS53 chained overlay -> javac against pinned prebuilt classes.
# Forge checkout consumed READ-ONLY. No WS48/WS50/shared file is modified.
# WS47 contract is staged at RUN time from the frozen ref (see run log), never here.
set -euo pipefail
export JAVA_HOME=/usr/lib/jvm/java-17-openjdk-amd64
WS=/home/moeen/code/ws53-forge-convergence-native-progression
...
# Source lock is recorded in WS53_SOURCE_LOCK.md and the run log (HEAD printed
# by the caller before invoking this script); the script pins only Forge.
...
sha256sum "$out/java/forge/game/player/Ws23ForgeVerticalProvider.java" "$out/java/forge/game/player/Ws40SuccessorState.java" > "$EV/WS53_PROVIDER_DIGESTS.txt"
cat "$EV/WS53_PROVIDER_DIGESTS.txt"
echo WS53-SEQUENCE-BUILD-OK
