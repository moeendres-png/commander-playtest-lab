#!/usr/bin/env python3
"""WS62 successor decision-breadth runner (WS62-owned, qualification-only).

WS62 VERBATIM COPY of candidate-qualification/ws55r-forge-rqc3-impact-closure/
ws55r_breadth_runner_full.py at WS55R terminal e39c7de0, EXCEPT the marked WS62 deltas below. The WS55R source file is never modified. Grants no behavior credit (BEHAVIOR_CREDIT=0/107).

WS62 DELTA-1 (successor pin): FORGE_COMMIT/TREE bound to production successor a9a95db/2c18327 (was base.FORGE_COMMIT old pin). Fails loud unless staged runners file carries successor and journal carries successor.
WS62 DELTA-2 (probe-copy gate): strict byte-identical check replaced with successor-aware gate (staged may differ from committed ONLY in the two pin lines, and must carry successor; otherwise fail closed).
WS62 DELTA-3: output journals additionally stamp ws62_runner identity.

VERBATIM COPY of candidate-qualification/ws55-forge-mandatory-decision-breadth/ws55_breadth_runner.py
at WS55 terminal 47a1e714, EXCEPT the three marked WS55R deltas below. The WS55
source file is never modified. Grants no behavior credit (BEHAVIOR_CREDIT=0/107).

WS55R DELTA-1 (journal budget): `len(drv.journal) >= 1024` -> `>= 4096`.
  Rationale: the corrected C01/A04 fixtures need longer NATURAL_GAME_START
  games (5+ land drops + response turns) than WS55's 1024-frame transcript
  budget allows. Transcript-size bound only; no engine/provider/Decision
  semantics change. Stamped per journal as ws55r_journal_budget.
WS55R DELTA-2 (engine priority polling bound): behavior_env override sets
  COMMANDER_LAB_FORGE_STOP_AFTER_PRIORITY=2048 (WS55/base default 512).
  Rationale: same long-game necessity; harness configuration like
  --structural-cap. Stamped per journal as ws55r_stop_after_priority.
WS55R DELTA-3: output journals additionally stamp ws55r_runner identity.
"""

# WS55R: original WS55 docstring body removed (mechanics identical; see WS55R header).
...
# WS55R DELTA-2: raise the engine-side priority polling bound for longer
# natural games (harness configuration; no engine/provider/Decision change).
WS55R_STOP_AFTER_PRIORITY = "2048"
WS55R_JOURNAL_BUDGET = 4096
_orig_ws55r_behavior_env = base.behavior_env


def _ws55r_behavior_env(record, transport):
    e = _orig_ws55r_behavior_env(record, transport)
...
_WS62_BASE_COMMIT = base.FORGE_COMMIT
_WS62_BASE_TREE = base.FORGE_TREE
    ...
    try:
        with _tempfile.TemporaryFile(mode="w+t", encoding="utf-8") as err:
            ...
            for _ in range(4096):
                ...
                # WS55R DELTA-1: journal budget 1024 -> 4096 (longer natural games).
                if len(drv.journal) >= 4096:
...
def main() -> int:
    ...
    # WS62 DELTA-2: successor-aware probe-copy gate (staged must be committed + pin migration only).
    if _hl.sha256(committed).hexdigest() != _hl.sha256(staged).hexdigest():
        def _ws62_norm(b: bytes) -> bytes:
            t = b.decode("utf-8", errors="strict")
            for pin in ("66caae16015bd403bc0a52fa6689afb5508f74d0", "40fc8f29ce4de31a964972461db2b48b4221e07f", "a9a95db6662c2d28814390a9c0c2f986e39aa8b4", "2c18327f79e330f2ed167067166ffd42d61b0849"):
                t = t.replace(pin, "<PIN>")
            return t.encode()
        import hashlib as _ws62_hl
        if _ws62_hl.sha256(_ws62_norm(committed)).hexdigest() != _ws62_hl.sha256(_ws62_norm(staged)).hexdigest():
            raise SystemExit("runners probe copy diverges beyond successor pin migration")
        if b"a9a95db6662c2d28814390a9c0c2f986e39aa8b4" not in staged or b"66caae16015bd403bc0a52fa6689afb5508f74d0" in staged:
            raise SystemExit("WS62_RUNNER_STAGED_PIN_NOT_SUCCESSOR")
    ...
    # WS55R DELTA-3: stamp harness-configuration deltas for transparency.
    result["ws55r_runner"] = ("ws55r_breadth_runner_full.py "
                              "(WS55-verbatim except journal-budget 4096 + "
                              "STOP_AFTER_PRIORITY=2048)")
    result["ws55r_journal_budget"] = WS55R_JOURNAL_BUDGET
    result["ws55r_stop_after_priority"] = int(WS55R_STOP_AFTER_PRIORITY)
    result["ws55_deck"] = {"main": a.deck_main, "commander": a.deck_commander}
    result["ws62_runner"] = "ws62_breadth_runner.py (WS55R-verbatim except successor pin + gate)"
    result["ws62_forge"] = {"commit": FORGE_COMMIT, "tree": FORGE_TREE}
    if (result.get("forge") or {}).get("commit") != FORGE_COMMIT or (result.get("forge") or {}).get("tree") != FORGE_TREE:
        raise SystemExit("WS62_JOURNAL_PIN_MISMATCH")
