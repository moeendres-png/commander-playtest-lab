#!/bin/bash
# WS68 provider transport-remediation build (WS68-owned, qualification-only).
# Mechanically equivalent to the WS65 build chain (ws65_build.sh) with only
# path/provenance changes for this WS68 worktree and the REUSED exact Forge
# checkout /tmp/ws65-forge-src-a9a95db, PLUS the WS68 transport overlay
# (payCombatCost authoritative transport + autoPassCancel native concession
# offer) applied after the WS64 overlay. No other overlay, generator, or
# bootstrap semantic change. Order (binding): generate(ws40) -> bootstrap ->
# successor/state overlays -> WS48 behavior overlay (repaired) -> WS53 chained
# overlay -> WS55 breadth overlay -> WS62 concession-transport + stack-SA
# overlay -> WS64 countertype-singleton transport overlay -> WS68 transport
# remediation overlay -> javac against the ACCEPTED-pin fresh classes (never
# old-pin prebuilts as evidence).
...
# WS62/WS64-owned file is modified. CPL audit base files are staged from this
# worktree only.
set -euo pipefail
export JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64
WS=/home/moeen/code/ws68-forge-provider-transport-remediation
FORGE=/tmp/ws65-forge-src-a9a95db
EV=/tmp/ws68-ev
...
case "$cp" in
  *forge-66caae*|*/.ws48-r1e/forge-66caae*) echo "WS68_OLD_PIN_ON_CLASSPATH_FAIL_CLOSED" >&2; exit 1;;
esac
...
sha256sum "$out/java/forge/game/player/Ws23ForgeVerticalProvider.java" "$out/java/forge/game/player/Ws40SuccessorState.java" "$out/java/forge/game/player/Ws62ConcessionTransport.java" > "$EV/WS68_PROVIDER_DIGESTS.txt"
cat "$EV/WS68_PROVIDER_DIGESTS.txt"
echo WS68-CLEAN-BUILD-OK
