#!/bin/bash
# WS64 cleaned provider build (WS64-owned, qualification-only).
...
#   overlay -> WS62 concession-transport + stack-SA overlay -> WS64
#   countertype-singleton transport overlay -> javac against SUCCESSOR
...
# from the required fresh mvn compile (no source edits). No WS48/WS53/WS55/WS62-owned
# file is modified. CPL audit base files are staged from this worktree only.
set -euo pipefail
export JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64
WS=/home/moeen/code/ws64-forge-a04-vertical-path-investigation
FORGE=/tmp/ws62-forge-src-a9a95db
EV=/tmp/ws64-ev-clean
...
case "$cp" in
  *forge-66caae*|*/.ws48-r1e/forge-66caae*) echo "WS64_OLD_PIN_ON_CLASSPATH_FAIL_CLOSED" >&2; exit 1;;
esac
...
sha256sum "$out/java/forge/game/player/Ws23ForgeVerticalProvider.java" "$out/java/forge/game/player/Ws40SuccessorState.java" "$out/java/forge/game/player/Ws62ConcessionTransport.java" > "$EV/WS64_PROVIDER_DIGESTS.txt"
cat "$EV/WS64_PROVIDER_DIGESTS.txt"
echo WS64-CLEAN-BUILD-OK
