...
def main() -> None:
    ...
    # XMage may natively activate the only usable mana ability before asking which
    # already-produced mana should be committed to the unpaid cost. At that point the
    # engine-offered semantic option is mana_pool/red, while cancel_mana_payment is a
    # transaction/UI escape rather than the canonical Magic decision to cast Bolt.
    # Match the unique native red pool option fail-closed; never select by index/default.
    replace_exact(
        RUNNER,
