...
def drive_record(record: dict[str, Any], proc) -> dict[str, Any]:
    ...
    def on_frame(sess: Session, frame: dict[str, Any]) -> None:
        ...
        if kind == "mulliganKeepHand":
            ...
            if expected is None or expected["decision_family"] != "mulligan":
                raise BehaviorFailure(f"UNSCRIPTED_MULLIGAN:{frame['payload'].get('decision_id')}")
...
def main() -> int:
    ...
    for index, fid in enumerate(ids, 1):
        ...
        try:
            ...
            detail = drive_record(record, proc)
