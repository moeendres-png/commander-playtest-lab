...
def _audit_record(record: dict[str, Any]) -> dict[str, Any]:
    ...
    contract_seed = randomness.get("rules_seed")
    if not isinstance(contract_seed, int) or isinstance(contract_seed, bool):
        raise RuntimeError(f"WS42_RULES_SEED_INVALID:{record['fixture_id']}:{contract_seed!r}")
    if scenario.get("seed") != contract_seed:
        raise RuntimeError(
            f"WS42_EXECUTION_SEED_MISMATCH:{record['fixture_id']}:"
            f"{scenario.get('seed')}:{contract_seed}"
        )
    ...
    return {
        ...
        "scenario_execution_seed": scenario["seed"],
        "rules_seed_exact": True,
...
def main() -> int:
    ...
    output = {
        "schema_version": "commander-lab.ws42-translation-integrity/1.0.0",
        ...
        "all_rules_seeds_exact": all(row["rules_seed_exact"] for row in rows),
    ...
    print(json.dumps({
        ...
        "all_rules_seeds_exact": output["all_rules_seeds_exact"],
    }, sort_keys=True))
    return 0
