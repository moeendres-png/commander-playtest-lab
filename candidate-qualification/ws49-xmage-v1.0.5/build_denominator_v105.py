#!/usr/bin/env python3
"""Independently reconstruct WS-49's exact immutable v1.0.5 provider denominator."""
...
def main() -> int:
    ...
    parser.add_argument("--published-denominator", type=Path)
    ...
    if dict(family_counts) != EXPECTED_FAMILIES:
        raise SystemExit(f"WS49_FAMILY_COUNTS_MISMATCH:{dict(family_counts)}")

    rows = []
    for ordinal, record in enumerate(records, 1):
        computed = requested_state_digest(record)
        if computed != record["requested_state_digest"]:
            raise SystemExit(f"WS49_REQUESTED_STATE_RECOMPUTE_MISMATCH:{record['fixture_id']}")
        rows.append({
            "ordinal": ordinal,
    ...
    if args.published_denominator:
        ...
        if not published_match:
            raise SystemExit("WS49_PUBLISHED_DENOMINATOR_POSTHOC_CROSSCHECK_MISMATCH")

    output = {
        "artifact_version": "commander-lab.ws49-denominator-manifest/1.0.0",
        "derivation": "immutable 135-record v1.0.5 materialization filtered independently by fixture_family != actual_card OR fixture_id == CARD_02",
        "published_ws47_provider_denominator_used_for_derivation": False,
        "published_ws47_provider_denominator_posthoc_match": published_match,
        "published_ws47_provider_denominator_sha256": published_sha,
