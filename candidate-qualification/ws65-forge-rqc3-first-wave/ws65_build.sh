#!/bin/bash
# WS65 fresh provider build (WS65-owned, qualification-only).
# Mechanically equivalent to the WS64 build chain (ws64_build.sh) with only
# path/provenance changes for this WS65 worktree and the exact Forge checkout
# /tmp/ws65-forge-src-a9a95db. No overlay, generator, or bootstrap semantic
# change. Order (binding): generate(ws40) -> bootstrap -> successor/state
# overlays -> WS48 behavior overlay (repaired) -> WS53 chained overlay ->
# WS55 breadth overlay -> WS62 concession-transport + stack-SA overlay ->
# WS64 countertype-singleton transport overlay -> javac against SUCCESSOR
...
WS=/home/moeen/code/ws65-forge-rqc3-first-wave
...
case "$cp" in
  *forge-66caae*|*/.ws48-r1e/forge-66caae*) echo "WS65_OLD_PIN_ON_CLASSPATH_FAIL_CLOSED" >&2; exit 1;;
esac
...
sha256sum "$out/java/forge/game/player/Ws23ForgeVerticalProvider.java" "$out/java/forge/game/player/Ws40SuccessorState.java" "$out/java/forge/game/player/Ws62ConcessionTransport.java" > "$EV/WS65_PROVIDER_DIGESTS.txt"
cat "$EV/WS65_PROVIDER_DIGESTS.txt"
echo WS65-CLEAN-BUILD-OK
