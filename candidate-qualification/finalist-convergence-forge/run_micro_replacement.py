#!/usr/bin/env python3
"""Execute exact v1.0.1 MICRO_REPLACEMENT through the pinned Forge provider."""
...
def submit_pass(proc: subprocess.Popen[str], frame: dict[str, Any]) -> None:
    options = frame.get("payload", {}).get("options", [])
    matches = [item for item in options if option_kind(item) == "PASS"]
    if len(matches) != 1:
        raise RuntimeError(f"PASS_OPTION_NOT_UNIQUE:{options}")
    ...
    proc.stdin.write(json.dumps({
        ...
        "payload": {"decision_id": frame["payload"]["decision_id"], "option_id": matches[0]["option_id"]},
    }, separators=(",", ":")) + "\n")
    proc.stdin.flush()
...
def validate_setup(snapshot: dict[str, Any]) -> None:
    base = snapshot["base"]
    if int(base["player_count"]) != 4 or str(base["turn"]) != "1" or str(base["phase"]) != "COMBAT_DAMAGE":
        raise AssertionError(f"native temporal setup mismatch:{base}")
    if base.get("active_actor") != "seat-1":
        raise AssertionError(f"native active player mismatch:{base}")
    players = {p["actor_id"]: p for p in base["players"]}
    expected = {
        "seat-1": "Gratuitous Violence|Grizzly Bears|Hill Giant",
        "seat-2": "Grizzly Bears",
        "seat-3": "Grizzly Bears",
        "seat-4": "",
    }
    for actor, names in expected.items():
    ...
    if snapshot.get("attacker") != "obj:micro-3power" or snapshot.get("attacker_power") != 3:
        raise AssertionError(f"native attacker mismatch:{snapshot}")
    if snapshot.get("defender") != "P2" or snapshot.get("combat_phase") != "COMBAT_DAMAGE":
        raise AssertionError(f"native combat assignment mismatch:{snapshot}")
...
def run_one(record: dict[str, Any], command: list[str]) -> dict[str, Any]:
    ...
    setup_snapshot = None
    result_message = None
    provider_internal_priority_passes = 0

    with tempfile.TemporaryFile(mode="w+t", encoding="utf-8") as stderr:
        proc = subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=stderr, text=True, env=env, bufsize=1)
        assert proc.stdin is not None and proc.stdout is not None
        proc.stdin.write(json.dumps({
            "protocol": PROTOCOL,
            "message_type": "CREATE_SESSION",
            "request_id": "forge-micro-replacement",
        ...
        for _ in range(512):
            ...
            if mtype == "DECISION_FRAME":
                if setup_snapshot is None:
                    ...
                    if len(matches) != 1:
                        raise RuntimeError(f"BOOTSTRAP_OPTION_NOT_UNIQUE:{kind}:{options}")
                    assert proc.stdin is not None
                    proc.stdin.write(json.dumps({"protocol":PROTOCOL,"message_type":"SUBMIT_DECISION","request_id":"reply-bootstrap","session_id":message.get("session_id"),"payload":{"decision_id":message["payload"]["decision_id"],"option_id":matches[0]["option_id"]}},separators=(",",":"))+"\n")
                    proc.stdin.flush()
                    continue
                if message["payload"]["decision_kind"] != "priority":
                    raise RuntimeError(f"UNEXPECTED_PRODUCTION_DECISION:{message}")
                provider_internal_priority_passes += 1
                submit_pass(proc, message)
                continue
        ...
        try:
            ...
            raise RuntimeError("FORGE_MICRO_REPLACEMENT_PROVIDER_TIMEOUT") from exc
        stderr.seek(0); error_text = stderr.read()
    if rc != 0:
    ...
    if payload.get("stop_reason") != "FINALIST_MICRO_REPLACEMENT_TERMINAL":
        raise AssertionError(f"unexpected stop reason:{payload.get('stop_reason')}:events={payload.get('native_events')}:snapshot={payload.get('snapshot')}")
    events = [str(x) for x in payload.get("native_events", [])]
    required = [
        "MICRO_REPLACEMENT_NATIVE_POWER:3",
        "MICRO_REPLACEMENT_NATIVE_EFFECT:Gratuitous Violence",
        "MICRO_REPLACEMENT_NATIVE_DAMAGE:P2:6",
    ]
    missing = [x for x in required if x not in events]
    if missing:
        raise AssertionError(f"replacement native evidence missing:{missing}:{events}")
    terminal_players = {p["actor_id"]: p for p in payload["snapshot"]["players"]}
    if terminal_players["seat-2"]["life"] != 34:
        raise AssertionError(f"native replacement terminal life mismatch:{terminal_players['seat-2']}")
    ...
    return {
        ...
        "provider_internal_priority_pass_count": provider_internal_priority_passes,
        "event_tape": normalized_events,
        "native_event_evidence": events,
        "terminal_semantic_state": {"P2": {"life": 34}, "obj:micro-3power": {"zone": "battlefield"}},
...
def main() -> None:
    ...
    try:
        ...
        row = {"fixture_id":FIXTURE_ID,"status":"FAIL","record_digest":rows[0].get("materialization_digest"),"failure_signature":f"{type(exc).__name__}:{exc}"}
    payload = {
        "schema_version":"finalist-convergence-forge-micro-replacement/1.0.0",
        "contract_commit":CONTRACT_COMMIT,"contract_bundle_digest":CONTRACT_BUNDLE,
        "candidate_commit":os.environ.get("COMMANDER_LAB_CONVERGENCE_BRANCH_HEAD","UNKNOWN"),
        "forge_commit":FORGE_COMMIT,"forge_tree":FORGE_TREE,"rows":[row],"counts":{row["status"]:1},
    }
