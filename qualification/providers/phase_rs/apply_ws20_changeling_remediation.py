#!/usr/bin/env python3
"""Deterministic WS-20 local remediation for phase.rs at the pinned source lock.

This script intentionally modifies the candidate Rules Core, never the Commander Lab
adapter.  It has two modes:

* --repro-only: add the minimized WS-15 regression test without changing rules code;
  the pinned upstream must fail this test.
* default: replace the known lossy commander creature-type helper with a generalized
  Changeling-aware implementation and add positive/negative/zone regression tests.

The caller is expected to verify the exact upstream Git commit before invoking this
script.  The script also refuses to operate unless the exact WS-15 TODO/function body
is still present, preventing silent application to a changed upstream implementation.
"""
...
PINNED_COMMIT = "bc218c51cec9cc2cec56f5c4de7c72be3d8e331c"
...
FULL_TESTS = REPRO_TEST + r'''
    ...
    /// expands through the runtime creature-type authority and preserves printed types.
    #[test]
    ...
    /// CR 604.3 + CR 702.73a: the CDA functions in all zones.  Commander
    /// designation is independent of zone, so the helper must not become a
    /// battlefield-only Changeling implementation.
    #[test]
    ...
    /// Changeling must therefore expand from CardFace keywords as well as live objects.
    #[test]
...
/// authority avoids a named-card special case and tracks newly loaded creature
/// types without a second hard-coded catalog.
///
...
pub fn commander_creature_types(state: &GameState, player: PlayerId) -> Vec<String> {
        ...
        .filter(|obj| obj.is_commander && obj.owner == player)
    {
        if obj
            ...
            .contains(&crate::types::card_type::CoreType::Creature)
        {
            // CR 702.73a is layer-4 characteristic semantics.  `base_keywords`
            // preserves the copiable/printed CDA across later-layer ability
            // removal; `keywords` also covers a current copied/materialized face.
