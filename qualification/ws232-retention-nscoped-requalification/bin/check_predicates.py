...
def resolve_value(path: str):
    ...
    if path == "config/rules_engines.json#primary_engine.commit":
        return json.loads((REPO_ROOT / "config/rules_engines.json").read_text())["primary_engine"]["commit"]
    ...
    if path == "qualification/manifests/COMMON_FIXTURE_MANIFEST_v1.json#seed":
        rows = json.loads((REPO_ROOT / "qualification/manifests/COMMON_FIXTURE_MANIFEST_v1.json").read_text())["fixtures"]
    ...
    if path.endswith("#rules-seed-binding"):
        sess = (REPO_ROOT / "engine-bridge/src/main/java/org/commanderlab/xmage/XmageFullGameSession.java").read_text()
        ok = ("setRulesSeed" in sess and "setRequireExplicitSeed(true)" in sess
              and "getRulesRandomCalls" in sess)
        return "setRulesSeed+setRequireExplicitSeed(true)+getRulesRandomCalls" if ok else "MISSING_BINDING"
...
def main() -> int:
    ...
    manifest = json.loads((REPO_ROOT / "qualification/manifests/COMMON_FIXTURE_MANIFEST_v1.json").read_text())
    ...
    for p in preds["predicates"]:
        ...
        for b in p["binds"]:
            ...
            if kind in ("path_blob_sha256", "fixture_bytes_sha256",
                        "card_domain_entry", "rules_authority", "provider_identity"):
                if "#" in path and kind == "fixture_bytes_sha256":
                    fid = path.split("#")[1]
                    canon = json.dumps(manifest_rows[fid], sort_keys=True,
                                       separators=(",", ":")).encode()
                    actual = hashlib.sha256(canon).hexdigest()
                else:
                    actual = file_sha(path)
                match = (actual == b["expected_sha256"])
                bind_results.append({"path": path, "kind": kind,
                                     "match": match, "actual_sha256": actual,
                                     "expected_sha256": b["expected_sha256"]})
            else:
                actual = resolve_value(path)
                match = (actual == b["expected_value"])
                bind_results.append({"path": path, "kind": kind,
                                     "match": match, "actual_value": actual,
    ...
    out = {"schema_version": "ws232-retention-predicate-results-1.0.0",
           ...
           "results": results}
    (NS / "RETENTION_PREDICATE_RESULTS.json").write_text(json.dumps(out, indent=1, sort_keys=True) + "\n")
    print(f"evaluated={len(results)} static_pass={npass} static_fail={len(results)-npass}")
    for r in results:
        if r["static_verdict"] != "STATIC_PASS":
            print("FAIL:", r["predicate_id"],
                  [b for b in r["bind_results"] if not b["match"]])
