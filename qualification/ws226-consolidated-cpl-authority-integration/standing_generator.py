#!/usr/bin/env python3
"""WS226 deterministic qualification-standing (WS225 logic reused; path-constant-only adaptation) + admission generator.

Reads canonical contracts plus the explicit WS226 mapping layer (WS225 logic reused)
...
Usage (from repo root):
  python3 qualification/ws226-consolidated-cpl-authority-integration/standing_generator.py
...
WS226 = Path(__file__).resolve().parent
WS225 = WS226
...
WS226_COMMIT = "LOCAL-WS226-WORKTREE"
WS225_COMMIT = WS226_COMMIT
...
def main() -> None:
    ...
    for cand in CANDIDATES:
        ...
        standings[cand] = {
            "schema_version": "ws226-standing/1.0.0",
            "candidate": cand,
            "source_lock": src_lock,
            "generated_by": "qualification/ws226-consolidated-cpl-authority-integration/standing_generator.py",
            "g_gates": g_gates,
            "af_gates": af_gates,
    ...
    dump("FIXTURE_EVIDENCE_TRACE.json", {
        "schema_version": "ws226-fixture-evidence-trace/1.0.0",
        "generated_by": "qualification/ws226-consolidated-cpl-authority-integration/standing_generator.py",
    ...
    dump("FREEZE_READINESS_VIEW.json", {
        "schema_version": "ws226-freeze-readiness/1.0.0",
    ...
    dump("OPEN_BLOCKERS.json", {"schema_version": "ws226-open-blockers/1.0.0", "blockers": blockers})

    dump("G01_STATUS.json", {
        "schema_version": "ws226-g01-status/1.0.0",
        "gate": "G01",
        ...
        "ws222_note": "WS222 may be active. WS222 AUTHORITY_LOCK_v2 consumed (scoped PASS). WS225 G01 FAIL frozen as history; WS226 G01 PASS recomputed from exact v2 trace.",
    })

    assessments = {cand: eval_admission(cand, facts[cand]) for cand in CANDIDATES}
    dump("ADMISSION_ASSESSMENTS.json", {
        "schema_version": "ws226-admission-assessments/1.0.0",
        "generated_by": "qualification/ws226-consolidated-cpl-authority-integration/standing_generator.py",
        "assessments": assessments,
    })
    dry = {cand: assessments[cand] for cand in ("quorune", "argentum")}
    dump("WS219_ADMISSION_DRY_RUN.json", {
        "schema_version": "ws226-ws219-dry-run/1.0.0",
    ...
    bias = {
        "schema_version": "ws226-incumbency-bias-test/1.0.0",
    ...
    dump("VALIDATION.json", {
        "schema_version": "ws226-validation/1.0.0",
        "checks": [
    ...
    print(f"WS226 generated {len(trace_rows)} trace rows; outputs digest {digest[:16]}")
