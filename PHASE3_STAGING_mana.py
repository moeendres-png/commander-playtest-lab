...
def _land_bounds(policy: DeckDesignPolicy) -> tuple[int, int, int, int]:
    ...
    if corridor is None:
        return 33, 39, 0, 98
    pref_low = max(0, int(round(corridor.preferred_minimum)))
    pref_high = min(98, int(round(corridor.preferred_maximum)))
    hard_low = 0 if corridor.hard_minimum is None else max(0, int(round(corridor.hard_minimum)))
    hard_high = 98 if corridor.hard_maximum is None else min(98, int(round(corridor.hard_maximum)))
...
def derive_mana_base_policy(policy: DeckDesignPolicy) -> ManaBasePolicy:
    ...
    midpoint = max(1, int(round((pref_low + pref_high) / 2)))
    # These are soft search targets, not claims of exact castability. CURRENT_CONTROL retains the
    # pre-existing Phase-7 source targets; experimental policies scale them with their land range.
    if policy.policy_id == PolicyId.CURRENT_CONTROL:
        w, u, r = 16, 17, 13
        basic_low, basic_high = 12, 22
    else:
        w = max(10, int(round(midpoint * 0.42)))
        u = max(11, int(round(midpoint * 0.45)))
        r = max(8, int(round(midpoint * 0.32)))
        basic_low = max(3, int(round(midpoint * 0.16)))
        basic_high = max(basic_low, int(round(midpoint * 0.48)))
    ...
    return ManaBasePolicy(
        ...
        preferred_t1_untapped_sources=max(10, int(round(midpoint * 0.38))),
        preferred_flexible_sources=max(6, int(round(midpoint * 0.22))),
        preferred_maximum_tapped_lands=max(5, int(round(midpoint * 0.28))),
        utility_land_budget=max(5, int(round(midpoint * 0.22))),
    )


def whole_deck_mana_summary(deck: StructuralDeckProfile, analysis: DeckManaAnalysis) -> dict[str, object]:
    noncommanders = [card for card in deck.cards if card.oracle_name not in set(deck.commander_names)]
    ...
    colored = {key: int(value) for key, value in sorted(analysis.colored_sources.items())}
    t1 = {key: int(value) for key, value in sorted(analysis.t1_untapped_land_sources.items())}
    turn2 = analysis.turn_castability_support.get(2, {})
    # Soft structural support proxy only; actual opening-hand probability is delegated to Mulligan Lab.
    source_ratios = [
    ...
    return {
        ...
        "turn2_source_supported_share": float(turn2.get("source_supported_share", 1.0)),
        "ramp_count": ramp,
...
def mana_soft_score(summary: dict[str, object], policy: ManaBasePolicy) -> float:
    colored = Counter({str(k): int(v) for k, v in dict(summary.get("colored_sources", {})).items()})
    land_count = int(summary.get("land_count", 0))
    basic_count = int(summary.get("basic_count", 0))
    t1 = Counter({str(k): int(v) for k, v in dict(summary.get("t1_untapped_land_sources", {})).items()})
    flexible = int(summary.get("flexible_source_count", 0))
    tapped = int(summary.get("definitely_tapped_land_count", 0))
    turn2 = float(summary.get("turn2_source_supported_share", 0.0))
    ramp = int(summary.get("ramp_count", 0))
    selection = int(summary.get("selection_count", 0))
    ...
    return color_score * 0.35 + t1_score * 0.15 + flex_score * 0.10 + turn2 * 0.25 + velocity_bonus - tapped_penalty * 0.5 - basic_penalty * 0.2
