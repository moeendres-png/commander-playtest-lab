...
for r in b['records']:
    ...
    for s in r.get('stack_state',[]):
        ...
        rows.append({
            ...
            'controller':s.get('controller') or obj.get('controller'),
            ...
            'decisions':[{
                'actor':d.get('actor'),
                'family':d.get('decision_family'),
                'semantic_value':d.get('selection',{}).get('semantic_value'),
                'causal_step_id':d.get('causal_step_id'),
            } for d in r.get('decision_script',[])],
        })
print(json.dumps({'count':len(rows),'rows':rows},ensure_ascii=False,sort_keys=True,separators=(',',':')))
