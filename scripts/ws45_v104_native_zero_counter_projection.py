...
iterable entry.  Cumulative upkeep nevertheless gives Forge a native rules
basis for the age-counter dimension, and getCounters(AGE) returns its native
value (including zero).  This patch extends observation only; it never mutates
game state and never reads requested counter keys or card names.
"""
