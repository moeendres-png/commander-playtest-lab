...
def classes_for(row: dict[str, Any], oracle_text: str, type_line: str) -> list[str]:
    ...
    if "when this creature dies" in text or "when this permanent dies" in text or "when this creature enters" in text or "when this land enters" in text:
        classes.add("STATE_TRIGGER")
    if "sacrifice" in text and ("rather than pay" in text or "additional cost" in text or ":" in text):
    ...
    if "shuffle" in text:
        classes.add("SHUFFLE")
    if any(x in text for x in ("combat", "attacks", "attack", "blocks", "blocked")) or "combat_payoff" in roles:
    ...
    for a in rows:
        ...
        for b in rows:
            ...
            weak = (
                ...
                and float(b["historical_search_rank_or_archive_context"]["objective_prior"] or 0.0) >= float(a["historical_search_rank_or_archive_context"]["objective_prior"] or 0.0)
            )
            strict = (
                b["candidate_frontier_distance"] < a["candidate_frontier_distance"]
                or b["changed_slots"] < a["changed_slots"]
                or float(b["historical_search_rank_or_archive_context"]["objective_prior"] or 0.0) > float(a["historical_search_rank_or_archive_context"]["objective_prior"] or 0.0)
            )
            if weak and strict:
                dominated = True
                break
...
def main() -> None:
    ...
    lab = WholeDeckDesignLab(root); control = current_control_mainboard(root)
    ...
    for source in ledger["rows"]:
        current = assess_variant_mechanics(
            lab.context, control=control, candidate=source["exact_mainboard"], deck_hash=source["deck_hash"]
        )
        tactical = assess_tactical_variant_capabilities(
            lab.context, control=control, candidate=source["exact_mainboard"], deck_hash=source["deck_hash"]
        )
        route = str(current["required_next_evidence_layer"]); route_counts[route] += 1
        ...
        entry = {
            ...
            "structural_safe_cards": [x["oracle_name"] for x in changed if x.get("decision_safe") is True],
            "screening_only_cards": [x["oracle_name"] for x in changed if x.get("tier") == "APPROXIMATED_SCREENING_ONLY"],
            "tactical_required_cards": [x["oracle_name"] for x in changed if x.get("tier") == "TACTICAL_REQUIRED"],
            "external_required_cards": [x["oracle_name"] for x in changed if x.get("tier") == "EXTERNAL_RULES_REQUIRED"],
            "missing_capabilities": sorted({c for x in current["blocked_cards"] for c in x.get("missing_structural_capabilities", [])}),
        ...
        for b in current["blocked_cards"]:
            name = str(b["oracle_name"]); f = facts.get(name, {})
            oracle_text = str(f.get("oracle_text") or ""); type_line = str(f.get("type_line") or b.get("type_line") or "")
            cc = classes_for(b, oracle_text, type_line); classes.update(cc)
            if name not in all_blocked_card_rows:
                all_blocked_card_rows[name] = {
                    ...
                    "required_state": ["4-player game state", "relevant zones", "priority/stack when applicable"],
                    ...
                    "trigger_dependency": bool({"TRIGGER_ORDER", "TRIGGER_COPY", "STATE_TRIGGER"}.intersection(cc)),
                    "combat_dependency": bool({"COMBAT_LEGALITY", "BLOCKING", "DAMAGE_ASSIGNMENT"}.intersection(cc)),
                    ...
                    "minimum_required_evidence_layer": (
                        "TACTICAL" if b.get("tier") == "TACTICAL_REQUIRED" else
                        "EXTERNAL_RULES" if b.get("tier") == "EXTERNAL_RULES_REQUIRED" else
    ...
    external_rows = [r for r in reclassified if r["current_fidelity_tier"] == "EXTERNAL_RULES_EVIDENCE_REQUIRED"]
    external_frontier = pareto_external(external_rows)
    for r in reclassified:
        r["near_frontier"] = r["deck_hash"] in external_frontier or r["search_archive_status"]["decision"] == "ELITE"
    ...
    baseline = {
        ...
        "external_runtime": {"xmage": "unavailable", "forge": "unavailable", "real_external_engine_executions": 0},
    ...
    dump(out / "HISTORICAL_EXTERNAL_ROUTE_RECLASSIFICATION.json", {
        "schema_version": "1.0.0",
    ...
    dump(out / "CARD_RULES_GAP_DECOMPOSITION.json", {
        "schema_version": "1.0.0",
    ...
    for capability in sorted(class_to_candidates):
        ...
        fully = sorted(
            h for h, req in candidate_classes.items() if req == {capability}
        )
        ...
        matrix.append({
            ...
            "evidence_layer": (
                "TACTICAL" if capability == "INSTANT_TIMING" else "TACTICAL_OR_EXTERNAL_DEPENDS_ON_CARD_SHAPE"
            ),
            ...
            "decision_archive_candidates_unlocked": [
                h for h in fully if row_by_hash[h]["search_archive_status"]["decision"] == "ELITE"
            ],
            ...
            "implementation_complexity": "LOW" if capability == "INSTANT_TIMING" else "UNKNOWN_UNTIL_CARD_SHAPE_BOUND",
            "validation_complexity": "LOW" if capability == "INSTANT_TIMING" else "UNKNOWN_UNTIL_CARD_SHAPE_BOUND",
            "existing_infrastructure_reuse": ["TacticalRuleOracle", "historical candidate ledger"],
            "new_infrastructure_required": [] if capability == "INSTANT_TIMING" else ["card-specific scenario contract and/or validated external engine"],
            "estimated_decision_value": "HIGHEST_FRONTIER" if capability == "INSTANT_TIMING" else ("FRONTIER_RELEVANT" if near else "SECONDARY"),
    ...
    for r in sorted((x for x in reclassified if x["near_frontier"]), key=lambda x: x["deck_hash"]):
        ...
        matrix.append({
            ...
            "cards_blocked": sorted({
                name for c in combo for name in class_to_cards[c]
            }),
            "candidate_count_affected": len(affected),
            "candidates_partially_improved": affected,
            "candidates_fully_unlocked": affected,
            "near_frontier_candidates_unlocked": [h for h in affected if row_by_hash[h]["near_frontier"]],
            "decision_archive_candidates_unlocked": [h for h in affected if row_by_hash[h]["search_archive_status"]["decision"] == "ELITE"],
            ...
            "new_infrastructure_required": ["multiple bounded card-shape contracts and/or validated external engine"],
    ...
    dump(out / "RULES_CAPABILITY_UNLOCK_MATRIX.json", {
        "schema_version": "1.0.0",
    ...
    minimum = {
        ...
        "still_blocked_candidate_count": len(reclassified) - sum(r["current_fidelity_tier"] == "STRUCTURAL_CONFIRMATORY_ALLOWED" for r in reclassified) - len(tactical_rows),
        "implementation_dependencies": ["TacticalRuleOracle basic_spell_timing", TACTICAL_CAPABILITY_CONTRACT_VERSION],
        "validation_dependencies": ["4P positive fixture", "4P negative fixture", "no-priority negative fixture", "3P fail-closed fixture"],
        ...
        "not_selected_capabilities": ["broad graveyard/recursion package", "spell-copy/alternative-cost package", "cascade/retrace package", "generic combat/trigger engine"],
    ...
    fixtures = [
        ("opt_opponent_turn_priority", {"spell_speed":"instant","player_count":4,"phase":"postcombat_main","actor_is_active":False,"has_priority":True,"stack_empty":False,"can_pay_cost":True}, True),
        ("preordain_opponent_turn_negative", {"spell_speed":"sorcery","player_count":4,"phase":"postcombat_main","actor_is_active":False,"has_priority":True,"stack_empty":True,"can_pay_cost":True}, False),
        ("preordain_own_main_positive", {"spell_speed":"sorcery","player_count":4,"phase":"precombat_main","actor_is_active":True,"has_priority":True,"stack_empty":True,"can_pay_cost":True}, True),
        ("opt_no_priority_negative", {"spell_speed":"instant","player_count":4,"phase":"precombat_main","actor_is_active":True,"has_priority":False,"stack_empty":True,"can_pay_cost":True}, False),
    ]
    fixture_rows=[]
    for fid,state,expected in fixtures:
        observed=oracle.evaluate("basic_spell_timing",state)
        fixture_rows.append({
            "fixture_id":fid,"capability_id":BASIC_INSTANT_TIMING_CAPABILITY,"players":4,
            "initial_state":state,"actor":"rogshai","seat":0,"priority_state":{"has_priority":state["has_priority"]},
            "stack_state":{"empty":state["stack_empty"]},"zones":{},"targets":[],"choices":[],
            "expected_legal_actions":["cast"] if expected else [],"expected_illegal_actions":[] if expected else ["cast"],
            "expected_final_state":{"can_cast":expected},"observed":observed,"engine_layer":"TACTICAL",
            "external_engine_used":False,"external_engine_version":None,"pass":observed["can_cast"] is expected,
        })
    dump(out / "RULES_EVIDENCE_GOLDEN_FIXTURES.json", {"schema_version":"1.0.0","fixtures":fixture_rows})
    dump(out / "TACTICAL_CAPABILITY_CONFORMANCE.json", {
        "schema_version":"1.0.0","tactical_runtime":TACTICAL_RULES_VERSION,
        ...
        "supported_choices":[],"supported_targets":[],"supported_zone_transitions":[],
        "unsupported_cases":["Flash-granting effects","split second","static cast restrictions/permissions","targets","modes","mana-generation decisions","strategic value of holding mana open"],
        "known_abstractions":["legality fixture only; not empirical winrate or External Rules evidence"],
        "golden_fixtures":[x["fixture_id"] for x in fixture_rows if x["pass"]],
        "negative_fixtures":[x["fixture_id"] for x in fixture_rows if not x["expected_final_state"]["can_cast"]],
    ...
    replay = {
        "schema_version":"1.0.0",
        "before":{"structural_safe":route_counts["STRUCTURAL_CONFIRMATORY_ALLOWED"],"tactical_required":route_counts["TACTICAL_EVIDENCE_REQUIRED"],"external_required":route_counts["EXTERNAL_RULES_EVIDENCE_REQUIRED"],"screening_only":route_counts["STRUCTURAL_SCREENING_ONLY"],"unsupported":route_counts["SEMANTIC_OR_MODEL_CAPABILITY_REQUIRED"]},
        "after":{"structural_safe":route_counts["STRUCTURAL_CONFIRMATORY_ALLOWED"],"tactical_evaluable":len(tactical_rows),"external_evaluable":0,"external_still_blocked":len(external_rows),"screening_only":route_counts["STRUCTURAL_SCREENING_ONLY"],"unsupported":route_counts["SEMANTIC_OR_MODEL_CAPABILITY_REQUIRED"]},
        "historical_external_candidates":EXTERNAL_HISTORICAL,
        ...
        "decision_elite_tactical_unlocked_ids":[r["deck_hash"] for r in tactical_rows if r["search_archive_status"]["decision"] == "ELITE"],
        "pass": True,
    ...
    dump(out / "RULES_EVIDENCE_VALIDATION_REPORT.json", {
        "schema_version":"1.0.0","phase":"pre_ci_local_validation",
        "tactical_runtime":TACTICAL_RULES_VERSION,"tactical_capability_contract":TACTICAL_CAPABILITY_CONTRACT_VERSION,
        "golden_fixtures_pass":all(x["pass"] for x in fixture_rows),
        "negative_fixtures_pass":all(x["pass"] for x in fixture_rows if not x["expected_final_state"]["can_cast"]),
        "post_unlock_routing_replay":"PASS","real_external_engine_executions":0,
        "external_scenario_classes_validated":0,
        "artifact_set_hash":sha256_value({"reclassified":reclassified,"minimum":minimum,"replay":replay}),
    })
