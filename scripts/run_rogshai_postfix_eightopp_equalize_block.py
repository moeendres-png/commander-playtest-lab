...
RUN_ID = "rogshai-postfix-eightopp-equalize-15360-2026-08-26-v1"
OPPONENT_IDS = {
...
EXPECTED_CANDIDATES = tuple(f"C{i:03d}" for i in range(1, 49))
...
def materialize_candidates(root: Path, payload: dict):
    ...
    if ids != EXPECTED_CANDIDATES:
        raise SystemExit("candidate ids must be exactly C001..C048")
    ...
    for row in rows:
        ...
        deck = context.materialize(mainboard, label=f"eightopp-{row['candidate_id'].lower()}")
...
def main() -> int:
    ...
    with results_path.open("w", encoding="utf-8", newline="\n") as results_file:
        for candidate_id, own in candidates.items():
            ...
            for row in schedule:
                ...
                triplet = [str(value) for value in row["opponent_triplet"]]
                observation = {
                    ...
                    "opponent_evidence_classes": {
                        opponent: OPPONENT_EVIDENCE_CLASSES[opponent] for opponent in triplet
                    },
                    ...
                    "opponent_information_class": (
                        "mixed_real_official_and_synthetic_proxy_equalization"
                    ),
                    "truth_boundary": (
                        "Structural model sample only; Cosmic is a synthetic public-deck proxy and "
                        "Morcant is a pool-constrained synthetic completion."
                    ),
                }
    ...
    report = {
        "schema_version": "rogshai-postfix-eightopp-equalization-block-report-1.0.0",
        "run_id": RUN_ID,
        "evidence_class": EVIDENCE_CLASS,
        "engine_version": ENGINE_VERSION,
        "fidelity_engine_version": FIDELITY_ENGINE_VERSION,
        ...
        "cosmic_proxy_used": True,
        "morcant_synthetic_completion_used": True,
