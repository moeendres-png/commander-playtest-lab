...
def _pilot_mode_adapter(label: str) -> tuple[str, str]:
    ...
    if label == "heuristic":
        return (
            "deterministic",
            "sealed heuristic label maps to deterministic decision mode; heuristic quality is represented by pilot_strength",
        )
...
def validate_preconditions() -> dict[str, object]:
    ...
    if (
        seal.get("sha256") != HOLDOUT_SHA
        or seal.get("outcomes_evaluated") is not False
        or seal.get("first_evaluation_status") != "not_run"
    ):
    ...
    if (
        freeze.get("holdout_sha256") != HOLDOUT_SHA
        or freeze.get("holdout_outcomes_seen") is not False
    ):
    ...
    if (
        finalists.get("status") != "frozen_pre_holdout"
        or finalists.get("holdout_outcomes_seen") is not False
    ):
        raise RuntimeError("finalists are not frozen pre-holdout")
    holdout = json.loads(HOLDOUT_PATH.read_text(encoding="utf-8"))
    if (
        holdout.get("status") != "sealed_untouched"
        or holdout.get("outcomes_evaluated_at_seal") is not False
    ):
...
def _build_variant(service: CommanderToolService, row: dict[str, object]):
    ...
    if not built.constraint_report.valid:
        raise RuntimeError(
            f"frozen finalist failed constraints: {deck_id}: {built.constraint_report.issues}"
        )
...
def _aggregate(deck_rows: list[dict[str, object]], all_effects: list[float]) -> dict[str, object]:
    ...
    return {
        ...
        "all_pair_randomization_p_value": paired_randomization_p_value(
            all_effects, seed=2026081106
        ),
        ...
        "pod_size_effects": {
            str(size): fmean(
                float(row["paired"]["placement_improvement"])
                for row in deck_rows
                if int(row["pod_size"]) == size
            )
            for size in (3, 4, 5)
        },
        "seat_effects": {
            str(seat): fmean(
                float(row["paired"]["placement_improvement"])
                for row in deck_rows
                if int(row["starting_player_seat"]) == seat
            )
            for seat in sorted({int(row["starting_player_seat"]) for row in deck_rows})
        },
        "opponent_uncertainty_effects": [
            float(row["paired"]["placement_improvement"])
            for row in deck_rows
            if row["uncertainty_axis"] == "opponent_uncertainty"
        ],
    }
...
def run() -> dict[str, object]:
    ...
    aggregate = {
        deck: _aggregate(rows, pair_effects_by_deck[deck]) for deck, rows in rows_by_deck.items()
    }
    ...
    for deck_id, adj_p in zip(decks, adjusted, strict=True):
        ...
        passed = (
            ...
            and float(agg["scenario_quantiles"]["q25"])
            >= float(gate["scenario_q25_effect_minimum"])
        ...
        recommendation_traces.append(
            build_recommendation_trace(
                ...
                baseline_identity={
                    "deck_id": deck_id,
                    "deck_hash": service._deck(deck_id).deck_hash,
                },
                variant_identity={
                    "variant_id": finalist["id"],
                    "deck_hash": finalist["variant_hash"],
                },
                ...
                recommendation_confidence_value="holdout_supported_model_internal"
                if passed
                else "not_supported_by_holdout",
            )
        )
    ...
    output = {
        ...
        "holdout_identity": {
            ...
            "sha256": HOLDOUT_SHA,
            "scenario_count": holdout["scenario_count"],
        },
        "preconditions": pre,
        ...
        json.dumps(
            {
                ...
                "aggregate_results": aggregate,
                "recommendation_gate": {
                    trace["baseline_identity"]["deck_id"]: trace["holdout_status"]
